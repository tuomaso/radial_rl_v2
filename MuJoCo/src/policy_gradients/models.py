...
class CtsPolicy_stdv(nn.Module):
    '''
    ...
    def __init__(self, state_dim, action_dim, init, hidden_sizes=HIDDEN_SIZES,
        ...
        if use_merged_bias:
            ...
            self.final_logstdv = nn.Linear(prev_size + 1, action_dim, bias=False)
        else:
            self.final_mean = nn.Linear(prev_size, action_dim, bias=True)
            self.final_logstdv = nn.Linear(prev_size, action_dim, bias=True)
        initialize_weights(self.final_mean, init, scale=0.01)
        initialize_weights(self.final_logstdv, init, scale=0.01)
        # For the case where we want to share parameters 
        # between the policy and value networks
        self.share_weights = share_weights
        if share_weights:
            ...
            initialize_weights(self.final_value, 0*init, scale=1.0)
    ...
    def forward(self, x):
        ...
        std = ch.exp(self.final_logstdv(x))
